...
- [Project Notification Scripts](#project-notification-scripts)
...
## Project Notification Scripts

This repository uses Apprise directly and ships one wrapper for consistency:

- `scripts/ci/notification/ci-20-send-notification.sh` — used in CI workflows or locally.

The script accepts `TITLE`, `MESSAGE`, and `TYPE` (`info|success|warning|failure`) as positional arguments and requires `APPRISE_URLS` (space-separated Apprise URLs) to be set in the environment or CI secrets. GitHub Actions context (repository, workflow, run URL, actor) is included automatically when available.

Example:

```bash
APPRISE_URLS="tgram://BOT/CHAT slack://TOKEN_A/TOKEN_B/TOKEN_C" \
  ./scripts/ci/notification/ci-20-send-notification.sh \
  "Build #123" "Tests passed" success
```
