...
DETECT_CHANGES_RESULT="${RESULT_DETECT_CHANGES:-unknown}"
...
ENABLE_DETECT_CHANGES="${ENABLE_DETECT_CHANGES:-false}"
...
ci:param report "RESULT_DETECT_CHANGES" "$DETECT_CHANGES_RESULT"
ci:param report "DETECTED_GATES" "$DETECTED_GATES"
...

{
    ...
    echo "| Detect Changes | $DETECT_CHANGES_RESULT | $ENABLE_DETECT_CHANGES |"
    ...
    if [ "$ENABLE_DETECT_CHANGES" = "true" ]; then
        echo ""
        echo "Fired change gates: \`${DETECTED_GATES:-none}\`"
    fi
} >> "${GITHUB_STEP_SUMMARY}"
