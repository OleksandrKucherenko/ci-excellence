#!/usr/bin/env bash
# Git Hooks Test Runner Script
# Convenience wrapper that calls the unified test runner for hooks domain
...
echo -e "${BLUE}=== Git Hooks Test Runner ===${NC}"
...
# Forward all arguments to the unified runner with hooks domain
exec "$UNIFIED_RUNNER" hooks "$@"