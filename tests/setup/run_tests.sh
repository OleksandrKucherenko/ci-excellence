#!/usr/bin/env bash
# Setup Tests Runner Script
# Convenience wrapper that calls the unified test runner for setup domain
...
echo -e "${BLUE}=== Setup Test Runner ===${NC}"
...
# Forward all arguments to the unified runner with setup domain
exec "$UNIFIED_RUNNER" setup "$@"